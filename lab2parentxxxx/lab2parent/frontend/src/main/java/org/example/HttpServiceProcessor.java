...
public class HttpServiceProcessor implements Runnable {
    ...
    public void run() {
        try {
            ...
            // process the request based the HTTP method.
    ...
    private void handleGetRequest(OutputStream output, String path) throws IOException {
        // Check if the request path starts with "/stocks/".
        if (path.startsWith("/stocks/")) {
            String stockName = path.substring(8);
            try {
                JSONObject stockDetails = getStockDetails("http://localhost:9001/stocks/" + stockName);

                if (stockDetails != null) {
                    sendJsonResponse(output, stockDetails, 200);
                } else {
                    sendErrorResponse(output, "Stock not found", 404);
                }
            } catch (IOException | JSONException e) {
                sendErrorResponse(output, e.getMessage(), 500);
            }
        } else {
    ...
    private void handlePostRequest(OutputStream output, String path, String body) throws IOException {
        if (path.startsWith("/order")) {
            ...
            URL url = new URL("http://localhost:9002/order");
    ...
    /**
     * 
     * Get stock details from an API endpoint.
     * 
     * @param urlStr The URL of the API endpoint.
     * 
     * @return The stock details as a JSON object.
     * 
     * @throws IOException   if an I/O error occurs while getting the stock details.
     * 
     * @throws JSONException if there is an error parsing the JSON response.
     */
    private JSONObject getStockDetails(String urlStr) throws IOException, JSONException {
